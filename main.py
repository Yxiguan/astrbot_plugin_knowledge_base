...
/kb help - 显示此帮助信息
    ...
    async def kb_add_file(
        ...
        """
        从本地路径或 URL 添加文件内容到知识库 (支持 .txt, .md)。
        用法: /kb add file <文件路径或URL> [可选的知识库名称]
        """
        if not await self._ensure_initialized():
            yield event.plain_result("知识库插件未初始化，请联系管理员。")
            return
        ...
        if not path_or_url:
            yield event.plain_result("请输入文件路径或 URL。")
            return

        file_to_process = None
        original_filename = "unknown_file"
        is_temp_file = False  # 标记是否是下载的临时文件，处理完后需删除
        ...
        except ValueError:  # 如果 path_or_url 格式不正确，urlparse 可能报错
            is_url = False

        if is_url:
            yield event.plain_result(f"检测到 URL，正在尝试下载: {path_or_url} ...")
            # 创建一个临时目录来存放下载的文件
            # 使用插件的持久化数据目录下的 "temp_downloads" 子目录
            ...
            logger.warning(
                f"用户提供了本地文件路径: {path_or_url}。请注意潜在的安全风险，确保路径是可信的。"
            )
            if os.path.exists(path_or_url) and os.path.isfile(path_or_url):
                file_to_process = path_or_url
                original_filename = os.path.basename(path_or_url)
            else:
                yield event.plain_result(f"本地文件路径无效或文件不存在: {path_or_url}")
                return

        if not file_to_process:
            yield event.plain_result("未能获取到要处理的文件。")
            return

        yield event.plain_result(f"收到文件 '{original_filename}'，正在解析...")

        content = await parse_file_content(
            file_to_process
        )  # parse_file_content 应该能处理路径
        if content is None:
            yield event.plain_result(
                f"无法解析文件 '{original_filename}'，支持的格式为 .txt, .md。"
            )
            if is_temp_file:
                try:
                    os.remove(file_to_process)
                    logger.info(f"已删除临时下载文件: {file_to_process}")
                except OSError as e_rm:
                    logger.error(f"删除临时文件失败 {file_to_process}: {e_rm}")
            return
        ...
        if self.config.get(
            ...
            try:
                ...
                if is_temp_file:
                    os.remove(file_to_process)  # 出错也清理临时文件
                return

        chunks = self.text_splitter.split_text(content)
        if not chunks:
            yield event.plain_result(f"文件 '{original_filename}' 分割后无有效内容。")
            if is_temp_file:
                os.remove(file_to_process)
            return
        ...
        try:
            yield event.plain_result(
                f"正在处理文件 '{original_filename}' ({len(chunks)} 个文本块) 并添加到知识库 '{target_collection}'..."
            )
            doc_ids = await self.vector_db.add_documents(
                target_collection, documents_to_add
            )
            if doc_ids:
                yield event.plain_result(
                    f"成功从文件 '{original_filename}' 添加 {len(doc_ids)} 条知识到 '{target_collection}'。"
                )
            else:
                yield event.plain_result(
                    f"未能从文件 '{original_filename}' 添加任何知识到 '{target_collection}'，请检查日志。"
                )
        except Exception as e:
            logger.error(
                f"从文件添加知识到知识库 '{target_collection}' 失败: {e}", exc_info=True
            )
            yield event.plain_result(f"添加知识失败: {e}")
        finally:
            if is_temp_file:
